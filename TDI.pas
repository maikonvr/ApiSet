...
uses ComCtrls, Forms, Messages, Controls, Menus, Classes, Types;
...
type
  ...
  private
    ...
    procedure CriarFormulario(Classe: TFormClass; vaImageIndex : Integer = 0);
    procedure CriarPageControl(ImageList: TImageList);
  ...
  public
    constructor Create(AOwner: TWinControl; aFormPadrao: TFormClass; aImageList: TImageList); reintroduce;
    destructor Destroy; override;

    procedure MostrarFormulario(Classe: TFormClass; Multi: Boolean = False; vaImageIndex : Integer = 0);
    procedure CriarModal(Classe: TFormClass);
...
uses SysUtils, Windows, pPrincipal;
...
procedure TTDI.MostrarFormulario(Classe: TFormClass; Multi: Boolean = False; vaImageIndex : Integer = 0);
begin
  ...
  CriarFormulario(Classe, vaImageIndex);
end;

constructor TTDI.Create(AOwner: TWinControl; aFormPadrao: TFormClass; aImageList: TImageList);
begin
  ...
  CriarPageControl(aImageList);

  FFormPadrao := aFormPadrao;
  if Assigned(FFormPadrao) then
  begin
    MostrarFormulario(FFormPadrao, False, 2);
  end;
end;
...
begin
  ...
  begin
    ...
    if Assigned(FFormPadrao) and not Assigned(PageControl.ActivePage) then
      MostrarFormulario(Value, False, 2);
  end;
end;
...
begin
  ...
  begin
    MostrarFormulario(FormPadrao, False, 2);
...
procedure TTDI.CriarFormulario(Classe: TFormClass; vaImageIndex : Integer = 0);
...
begin
  ...
  begin
    ...
    PageControl.ActivePage.ImageIndex := vaImageIndex;
...
procedure TTDI.CriarModal(Classe: TFormClass);
begin
  Application.CreateForm(Classe, TForm(Classe));
  TForm(Classe).ShowModal;
  FreeAndNil(TForm(Classe));
end;

procedure TTDI.CriarPageControl(ImageList: TImageList);
begin
